...
export class ReputationQueryService {
  ...
  private async processQuery(resource: string): Promise<ReputationResult> {
    ...
    try {
      ...
      if (statusCode === 200) {
        const data = this.accept === 'application/json'
      ...
      const message = ERROR_MESSAGES[statusCode] ||
  ...
  private async queryList(list: string, resource: string): Promise<ReputationResult> {
