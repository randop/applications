...
seastar::future<> write_file(size_t core_num) {
    ...
    std::string filename = std::string("file_") + std::to_string(core_num) + ".dat";
    return open_file_dma(
        filename,
        open_flags::create | open_flags::truncate | open_flags::rw
        ...
        return seastar::do_with(WriteState(std::move(f), core_num), [total_size, chunk_size] (WriteState& state) {
            return seastar::do_until([&state, total_size] { return state.pos >= total_size; },
                [&state, chunk_size] () {
                    size_t to_write = std::min(chunk_size, total_size - state.pos);
                    return state.f.dma_write(state.pos, state.buffer.data(), to_write, nullptr).then([&state, to_write] (size_t result) {
                        if (result == 0) {
