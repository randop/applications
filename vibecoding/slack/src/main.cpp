...
json::value httpsPostJson(const std::string& host,
...
std::string openSocketModeUrl(const std::string& appToken) {
    json::value res = httpsPostJson(
        "slack.com", "/api/apps.connections.open", appToken,
        json::value(json::object{}));

    const json::object& obj = res.as_object();
    if (!getBool(obj, "ok"))
        throw std::runtime_error(
            "apps.connections.open failed: " + json::serialize(res));
...
public:
    SocketModeSession(net::io_context& ioc,
                      ssl::context& sslCtx,
                      std::string botToken,
                      std::string wsUrl,
...
private:
    ...
    void onConnect(beast::error_code ec) {
        ...
        if (!SSL_set_tlsext_host_name(
                ws_.next_layer().native_handle(), parsed_.host.c_str())) {
        ...
        ws_.set_option(websocket::stream_base::decorator(
            [](websocket::request_type& req) {
                req.set(http::field::user_agent,
                        "boost-beast-slack-bot/1.0");
            }));

        ws_.async_handshake(
            parsed_.host, parsed_.target,
            net::bind_executor(
                strand_,
                [self = shared_from_this()](beast::error_code ec) {
    ...
    void sendBootNotification() {
        ...
        net::post(httpPool_,
            [weakSelf, botToken, message = std::move(message)]() mutable {
                try {
                    json::value res = httpsPostJson(
                        "slack.com", "/api/chat.postMessage",
                        botToken, json::value(std::move(message)));

                    if (auto self = weakSelf.lock()) {
                        net::post(self->strand_,
                            [self, res = std::move(res)]() mutable {
                                if (!res.is_object() ||
                                    !getBool(res.as_object(), "ok")) {
                                    log("Boot notification failed: " +
                                        json::serialize(res));
                                }
                            });
                    }
    ...
    void handleEnvelope(const json::object& envelope) {
        ...
        if (type == "disconnect") {
            log("Received disconnect envelope (reason: " +
                getStr(envelope, "reason", "unknown") +
                "); reconnecting...");
            reconnect_ = true;
            stop();
            return;
    ...
    void handleMessageEvent(const json::object& event) {
        ...
        constexpr bool willReplyOnThread = false;
        ...
        net::post(httpPool_,
            [weakSelf, botToken, reply = std::move(reply)]() mutable {
                try {
                    json::value res = httpsPostJson(
                        "slack.com", "/api/chat.postMessage",
            ...
            ws_.async_close(
                ...
                net::bind_executor(
                    ...
                    (beast::error_code closeEc) {
                        if (closeEc &&
                            closeEc != net::error::operation_aborted) {
    ...
    while (true) {
        try {
            ...
            auto session = std::make_shared<SocketModeSession>(
                ioc, ctx, botToken, wsUrl, httpPool);
