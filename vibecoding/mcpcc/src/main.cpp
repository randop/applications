...
void write_response(uv_stream_t *client, const json &response) {
  std::string response_str = response.dump() + "\n";
  uv_write_t *write_req = new uv_write_t();
  write_req->data = new char[response_str.size()];
  std::copy(response_str.begin(), response_str.end(),
...
void handle_initialize(uv_stream_t *client, const json &request) {
...
void handle_tools_list(uv_stream_t *client, const json &request) {
...
void handle_tools_call(uv_stream_t *client, const json &request) {
...
void process_request(uv_stream_t *client, const std::string &data) {
...
void on_new_connection(uv_stream_t *server, int status) {
  ...
  if (uv_accept(server, reinterpret_cast<uv_stream_t *>(client)) == 0) {
    uv_read_start(reinterpret_cast<uv_stream_t *>(client), alloc_buffer,
                  echo_read);
  } else {
