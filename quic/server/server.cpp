...
static inline socklen_t sa_len(const sockaddr *sa) {
  return sa->sa_family == AF_INET ? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
}
...
static lsquic_conn_ctx_t *on_new_conn(void *, lsquic_conn_t *conn) {
      ...
      if (data) {
        ...
        if (cqe->res > 0) {
          lsquic_engine_packet_in(engine, (unsigned char *)r->buf, cqe->res,
                                  (sockaddr *)&addr, (sockaddr *)&r->peer, NULL,
                                  0);
        }
